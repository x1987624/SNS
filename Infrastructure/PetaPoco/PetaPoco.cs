...
namespace PetaPoco
{
    ...
    public partial class Database : IDisposable
    {
        ...
        public Database(string connectionStringName)
        {
            ...
            //added by mazq,2011-10-14,如果不提供connectionStringName由原来的获取第一个connectionString改成最后一个connectionString
            // Use last?
            if (string.IsNullOrEmpty(connectionStringName))
            {
                int connectionStringsCount = ConfigurationManager.ConnectionStrings.Count;
                if (connectionStringsCount > 0)
                    connectionStringName = ConfigurationManager.ConnectionStrings[connectionStringsCount - 1].Name;
                else
        ...
        // mazq,2011-10-17,怀疑 “已有打开的与此 Command 相关联的 DataReader，必须首先将它关闭”的问题与原非即时返回查询结果有关，修改成直接返回所有查询结果
        /// <summary>
                    ...
                    lock (syncObj)
                    {
                        using (var cmd = CreateCommand(_sharedConnection, ""))
                        {
                            ...
                            foreach (var i in pd.Columns)
                            {
                                ...
                                //added by mazq 2012-9-19
                                if ((SqlBehaviorFlags.Insert & i.Value.SqlBehavior) == 0)
                                    continue;
                        ...
                        using (var cmd = CreateCommand(_sharedConnection, ""))
                        {
                            ...
                            if (columns == null)
                            {
                                foreach (var i in pd.Columns)
                                {
                                    ...
                                    //added by mazq 2012-9-19
                                    if ((SqlBehaviorFlags.Update & i.Value.SqlBehavior) == 0)
                                        continue;
    ...
    */
    ...
    namespace Internal
    {
        ...
        abstract class DatabaseType
        {
            ...
            //added by zhengw,2013-10-12,增加primaryKey参数
            /// <summary>
        ...
        internal class PocoColumn
        {
            ...
            public bool ResultColumn;
            //added by zhengw,2013-10-10,默认为true
            ...
            {
                get { return this.sqlBehavior; }
                set { this.sqlBehavior = value; }
            }
        }
        ...
        class PocoData
        {
            ...
            public PocoData(Type t)
            {
                ...
                {
                    //added by mazq,2011-10-10,跳过只读或只写属性 
                    ...
                    //added by mazq 2012-9-19
                    var sqlBehaviorAttrs = pi.GetCustomAttributes(typeof(SqlBehaviorAttribute), true);
                    if (sqlBehaviorAttrs.Length > 0)
                    {
                        SqlBehaviorAttribute sqlBehaviorAttr = sqlBehaviorAttrs[0] as SqlBehaviorAttribute;
                        if (sqlBehaviorAttr != null)
                            pc.SqlBehavior = sqlBehaviorAttr.Behavior;
                    }
            ...
            private static Func<object, object> GetConverter(IMapper mapper, PocoColumn pc, Type srcType, Type dstType)
            {
                ...
                else if (!dstType.IsAssignableFrom(srcType))
                {
                    ...
                    //added by mazq,2011-11-17, int类型0转换成bool类型false
                    else if (dstType.Equals(typeof(bool)))
                    {
                    ...
                    }//added by libsh，2011-11-04，处理字符串转换成Guid类型时抛异常问题
                    else if (dstType.Equals(typeof(Guid)))
                    {
                        return delegate(object src)
                        {
                            return new Guid(src.ToString());
                        };
                    }//added by libsh，2012-03-09，处理泛型类型的转换
                    else if (dstType.IsGenericType && dstType.GetGenericTypeDefinition() == typeof(Nullable<>))
                    {
                        return delegate(object src)
                        {
                            return Convert.ChangeType(src, Nullable.GetUnderlyingType(dstType));
                        };
                    }
                    else
                    {
        ...
        {
            //added by zhengw,2013-10-14,增加primaryKey参数
        ...
        internal static class PagingHelper
        {
            ...
            //added by zhengw,2013-10-14,修改OrderBy正则
    ...
    namespace DatabaseTypes
    {
        ...
        class SqlServerDatabaseType : DatabaseType
        {
            public override string BuildPageQuery(long skip, long take, PagingHelper.SQLParts parts, ref object[] args, string primaryKey = null)
            {
                ...
                //added by zhengw,2013-10-12,解决主键不能由多部分组成问题
                string primaryKeyPart = primaryKey ?? string.Empty;
                if (primaryKey.Contains('.') && !primaryKey.EndsWith("."))
                {
                    primaryKeyPart = primaryKey.Substring(primaryKey.LastIndexOf(".") + 1);
                }

                var sqlPage = string.Format("SELECT peta_paged.{4} FROM (SELECT ROW_NUMBER() OVER ({0}) peta_rn, {1}) peta_paged WHERE peta_rn>@{2} AND peta_rn<=@{3}",
                                        parts.sqlOrderBy == null ? "ORDER BY (SELECT NULL)" : parts.sqlOrderBy, parts.sqlSelectRemoved, args.Length, args.Length + 1, primaryKeyPart);
