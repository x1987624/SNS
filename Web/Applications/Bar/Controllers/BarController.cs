...
namespace Spacebuilder.Bar.Controllers
{
    ...
    /// 帖吧
    /// </summary>
    [TitleFilter(TitlePart = "帖吧", IsAppendSiteName = true)]
    ...
    public class BarController : Controller
    {
        ...
        public BarRatingService barRatingService { get; set; }
       ...
       [DonutOutputCache(CacheProfile = "Stable")]
        ...
        public JsonResult DeleteManager(long sectionId, long userId)
        {
            ...
            if (!authorizer.BarSection_SetManager(barSection))
        ...
        public ActionResult UserThreads(string spaceKey, bool isPosted = false, int pageIndex = 1)
        {
            ...
            bool ignoreAudit = UserContext.CurrentUser != null && UserContext.CurrentUser.UserId == user.UserId || authorizer.IsAdministrator(BarConfig.Instance().ApplicationId);
        ...
        public ActionResult Delete(long threadId)
        {
            ...
            if (authorizer.BarThread_Delete(thread))
            {
        ...
        public ActionResult _CreatBarRating(long threadId)
        {
            ...
            string errorMessage;
            if (!authorizer.BarRating(threadId, out errorMessage))
            {
        ...
        public ActionResult _CreatBarRating(BarRatingEditModel model)
        {
            ...
            string errorMessage;
            if (!authorizer.BarRating(thread, out errorMessage))
            {
                WebUtility.SetStatusCodeForError(Response);
        ...
        public ActionResult DeletePost(long postId)
        {
            ...
            if (authorizer.BarPost_Delete(post))
        ...
        //todo:bianchx,by zhengw:子回帖列表怎么自动展开？新回帖通知中，如果是子回帖怎么能定位到该子回帖位置？
        ...
        {
            if (!authorizer.BarSection_Manage(model.SectionId ?? 0))
            {
                return Redirect(SiteUrls.Instance().SystemMessage(TempData, new SystemMessageViewModel
                {
                    Title = "没有权限",
        ...
        public ActionResult ChangeDisplayOrder(long id, long referenceId)
        {
            ...
            {
                //todo:libsh,by zhengw:分类应该有ChangeDisplayOrder方法
        ...
        public ActionResult DeleteThreadCategory(long categoryId)
        {
            ...
            if (authorizer.BarSection_Manage(category.OwnerId))
            {
        ...
        public ActionResult BatchSetSticky(List<long> threadIds, bool isSticky, DateTime stickyDate)
        {
            ...
            foreach (var item in barThreads)
            {
                if (!authorizerCache.ContainsKey(item.SectionId))
                    authorizerCache.Add(item.SectionId, authorizer.BarSection_Manage(item.SectionId));
        ...
        public ActionResult BatchUpdatePostAuditStatus(List<long> postIds, bool isApproved = true)
        {
            ...
            Dictionary<long, bool> authorizers = new Dictionary<long, bool>();
            IEnumerable<BarPost> barPosts = barPostService.GetBarPosts(postIds);
            foreach (var post in barPosts)
            {
                if (!authorizers.ContainsKey(post.SectionId))
                    authorizers.Add(post.SectionId, authorizer.BarSection_Manage(post.SectionId));
                if (authorizers[post.SectionId])
