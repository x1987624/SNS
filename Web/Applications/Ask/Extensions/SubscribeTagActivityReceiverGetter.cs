...
namespace Spacebuilder.Ask
{
    ...
    /// 关注问题标签动态接收人获取器
    /// </summary>
    public class SubscribeTagActivityReceiverGetter : IActivityReceiverGetter
    {
        private SubscribeService subscribeService = new SubscribeService(TenantTypeIds.Instance().AskTag());
        ...
        IEnumerable<long> IActivityReceiverGetter.GetReceiverUserIds(ActivityService activityService, Activity activity)
        {
            ...
            //将动态发送者（问题的作者）从动态的接收对象中移除（否则如果动态发送者也关注了该标签，就会产生重复的动态数据）
